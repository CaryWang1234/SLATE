...
- 🛰️ **跨项目并行运行** —— 切换项目不再打断正在生成的对话：离开的那场在后台继续跑，进度与结局落进右栏任务中心；同一项目默认串行、跨项目最多两场同时（1–4 可调），超出上限的发送不会被吞——进队列并写明「排队中」，前一场一收工自动接上；每条会话的 token 各记各的，用量条只反映屏幕上这一场
- ⏳ **子代理可转后台** —— 模型一次派出多个并行子代理时可以选「转后台」：拿到任务编号就继续干别的活，这批子代理在后台跑完，各份结论以「后台任务消息」送回来叫醒模型；右栏「后台任务」面板同时显示两路后台（终端任务与子代理批次），那里可以看结论也可以就地停止。同时最多 3 批；这批活在浏览器页面里跑，刷新或关页面即止（`bg_task` 活在后端进程里，两者寿命不同）
- 🎯 **分项目覆盖（Actions / 知识库 / MCP）** —— 与分项目宪法同一套口径的"全局 + 项目覆盖"三件事：流程说明书可以在项目的 `.slate/actions/<id>.yml` 放一份同名的顶掉全局那份；知识按归属分文档（同名时项目那条顶掉全局那条，全局其余照旧可见，不会一进项目知识就消失）；MCP 服务器可以在项目里单独关掉或只放行几个工具——项目的 `.slate/config.json` 里只记这台开不开、哪些工具准用，URL 与密钥绝不落进你的仓库。列表标出哪一条是「本项目」那份，编辑器与设置页写明现在改的是哪一份，摘掉覆盖即回到跟全局
...
- 🛡️ **审批模式（每个对话可不同）** —— 三档：手动审批=AI 执行命令、访问网络前逐条问你；自动审批=只在命中 23 类高危规则时问你；完全访问=都不问。默认档在设置页，单个对话在输入框的审批胶囊里随时改（完全访问那一档整颗标红）；批准前由模型解释命令目的，灾难级命令（`rm -rf /`、`format` 等）任何模式下都无条件禁止
...
- 📚 **知识库**：本地知识片段检索与注入
- 🛡️ **终端安全**：高危命令写死规则判定，前端审批弹窗 + 后端独立拦截双层防御，审批档位可按对话分（手动逐条确认 / 自动只拦高危 / 完全访问不问），灾难级命令（`rm -rf /`、`format` 等）无条件禁止
