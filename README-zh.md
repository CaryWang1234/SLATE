...
- 🗂️ **工作区（多文件夹项目）** —— 一个项目可以同时收进多个互不相关的文件夹，同一时刻只在一个"当前根"上工作（文件、Git、终端都跟着它走），侧栏点一下即可切换
- 📜 **项目宪法分项目生效** —— 宪法可以只属于某个项目（写进它的 `.slate/config.json`），设置页与提示词工厂都标明现在改的是哪一份，项目没写时回落全局
...
- **Continue Autopilot（设置里可关，默认开）**：轮数用完不等于干完。末轮还在动手时，把上限往后推 8 轮（最多推 3 次）让它把手上的活做完；末轮空手停笔时，把清单剩余项念回给它，催它继续推进或明确写【任务完成】
...
- 大任务自动建立 TODOLIST（消息区右侧栏实时展示，顶栏「任务清单」按钮可随时折叠，折叠后按钮上仍报 `{done}/{total}` 进度），统筹全局批量推进，每完成一项或一批立即同步进度，未全部了结不得宣称完成
...
| `terminal` | 受限沙箱命令执行；Windows 子进程隐藏，原生命令输出支持中文与 emoji。Windows 下每条命令一个 PowerShell 进程：多行块与 `&&` / `||` 都可用，语法错误会带非零退出码原样返回，`cd` 与 `$env:` 跨命令保持 |
| `bg_task` | 后台终端任务：`action=start` 起完立刻返回（pid + 开头输出 + 日志路径），不占住对话；要结果用 `status` / `log` 主动问，或起任务时带 `notify=true`，任务结束（或命中你给的正则）时带着输出尾巴把模型叫醒；`action=stop` 杀整棵进程树。任务寿命等于后端进程寿命，日志落在 `data/bg_tasks/`，高危命令同样要审批 |
...
│   │   └── files.py            # 多模态文件解析
│   └── skills/                 # 35 个内置 MCP 工具实现（含 Unicode-safe 文件/终端工具、后台终端任务、高危命令双层拦截与可取消调用上下文）
├── frontend/
