...
它内置多模型对话、Agent Autopilot、MCP 工具调用、Harness 自主执行、磨墨模式、AI 团队辩论与白板式逻辑链，既能直接驱动内置工具完成任务，也能生成高质量 Prompt 交给外部 Coding Agent（Claude Code、Codex、Cursor 等）执行。
...
- ⚡ **Agent Autopilot + Harness** —— 普通项目任务会自动进入 Agent 循环，不再需要反复发送“继续”；Harness 仍是显式六阶段、50 轮强闭环模式
...
- 📡 **局域网遥控 + 鉴权** —— 应用启动即在 8001 端口开放局域网访问，手机 / 平板浏览器扫码即用同一界面；可设置局域网访问密码，避免同网段设备未经授权操作 SLATE
...
- 🧠 **升级版黑板** —— 卡片 + 连线整理思路，Mermaid 渲染 flowchart / mindmap，支持流程 / 看板 / 纲要 / Git 树等显示模式；工具执行自动记录为带状态颜色的步骤卡片
...
- 普通任务默认最多 18 轮，全面排查 / 项目级 / 多文件任务默认最多 28 轮；Harness 仍保留 50 轮强模式
