...
- ⚡ **Harness 自主执行** —— 六阶段闭环自主推进，大任务自动建 TODOLIST 逐项销账，带进度显示可随时停止
- 🛠️ **15 个内置 MCP 工具** —— 文件读写编辑、终端沙箱、仓库统计、正则测试、文本摘要、联网搜索与网页抓取等
...
- 大任务自动建立 TODOLIST（消息区右侧栏实时展示），逐项销账，未全部了结不得宣称完成
...
│   │   └── files.py            # 多模态文件解析
│   └── skills/                 # 15 个内置 MCP 工具实现（含高危命令双层拦截）
├── frontend/
...
├── installer/                  # 安装包产物
└── data/                       # 运行数据（SQLite / 宪法 / 定时任务 / 自定义 Skill / 专家包）
```
