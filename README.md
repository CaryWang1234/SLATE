...
- 🛡️ **Approval Modes (per conversation)** — three tiers: Manual asks before every command and every network call; Auto asks only when one of 23 hardcoded high-risk rules matches; Full Access asks nothing. The settings page holds the default for new conversations, and the approval pill in the input bar changes it for that conversation alone (Full Access renders red). The AI explains a command's purpose before you approve, and catastrophic commands (`rm -rf /`, `format`, etc.) are blocked in every mode
...
- 🛡️ **Terminal Security**: Hardcoded high-risk command rules, frontend approval + backend interception dual defense, a per-conversation approval tier (manual asks each call / auto asks only high-risk / full access asks nothing), catastrophic commands (`rm -rf /`, `format`, etc.) unconditionally blocked
