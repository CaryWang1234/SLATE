    ...
    if action.startswith("window_"):
        try:
            ...
            if action == "window_list":
                ...
                return {"status": "ok", "count": len(titles), "windows": titles[:50]}
            ...
            if win is None:
                ...
                return {"status": "not_found", "message": f"未找到标题包含「{title}」的窗口", "windows": visible}
            ...
            return {"status": "ok", "window": win.title, "message": f"已对窗口「{win.title}」执行 {op}"}
        except Exception as e:
    ...
    try:
        ...
        if action == "type":
            ...
            if any(ord(c) > 127 for c in text):
                ...
                pyperclip.copy(text)
                pag.hotkey('ctrl', 'v')
                return {"status": "ok", "message": f"已通过剪贴板粘贴文字 ({len(text)} 字符)"}
            else:
        ...
        if action == "press":
            ...
            n = max(1, int(repeats))
            pag.press(key, presses=n, interval=interval or 0.05)
            return {"status": "ok", "message": f"已按下按键 {key} x{n}"}
