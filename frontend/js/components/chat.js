...
import { state, subscribe, addMessage, updateLastAssistantMessage, setMessages, setConversations, getModelKey, addUsage, estimateContextTokens, resetUsage, restoreUsageForConversation, setConversationUsage, setKnowledgeContext, savePersistent, getConversationTodos, setConversationTodos, setActiveExpertId, addBoardCard } from "../store.js?v=20260818-80";
import { get, post, del, patch, streamChat, upload, REASONING_PREFIX } from "../services/api.js?v=20260818-80";
import { buildMessages, getDefaultParams, getOutputMaxTokens } from "../services/adapter.js?v=20260818-80";
import { detectToolCalls, stripToolCalls, executeToolCalls, hasTruncatedTail } from "../services/tools.js?v=20260818-80";
import { renderMarkdown } from "../services/markdown.js?v=20260818-80";
import { openMemoryModal, openSnippetModal, autoRefineMemoryAndProfile, captureConversationSpark } from "./memory.js?v=20260818-80";
import { getExpertsCached } from "./experts.js?v=20260818-80";
import { addToolStepCard, updateToolStepCard } from "./whiteboard.js?v=20260818-80";
import { loadExperts, getExpert, readExpertFile } from "../services/experts.js?v=20260818-80";
import { fmtTokens, tokenEquivalence } from "../services/usage.js?v=20260818-80";
import { fileTypeIcon } from "../services/file_icons.js?v=20260818-80";
import { dlgConfirm, dlgPrompt, dlgToast } from "../services/dialog.js?v=20260818-80";
import * as grindSvc from "../services/grind.js?v=20260818-80";
import { t } from "../services/i18n.js?v=20260818-80";
import { notifyTaskComplete } from "../services/notify.js?v=20260818-80";
...
1. 目标：开篇用一句话明确本次任务的目标与验收标准。
2. 计划：面临大任务（多步骤 / 多文件 / 复杂修改）必须先调用 todo_manage(action=init) 建立 TODOLIST，拆解为可执行、可验证的步骤；简单任务可跳过。
3. 执行：统筹清单全局推进，能并行的多项一起处理；每完成一项或一批事项，立即调用 todo_manage(action=update) 批量更新状态，让清单随时反映真实进度；受阻项标为 blocked 并说明原因；需要信息或操作时直接调用 MCP 工具。
4. 验证：每步完成后自行验证结果（重新读取文件 / 执行命令 / 核对输出），失败则修复后重新验证。
5. 汇报：全部完成后输出完结报告，逐条核对 TODOLIST（done / blocked + 原因），未全部了结不得宣称任务完成。
6. 追溯：用 1-3 句总结本次任务的关键决策、踩坑与可复用经验。
...
async function continueTruncatedOutput(msgEl, content, modelId, apiKey, baseUrl, params, signal, finishReason = "") {
  ...
  for (let round = 1; round <= MAX_CONTINUE_ROUNDS; round++) {
    ...
    try {
      const { toast } = await import("../app.js?v=20260818-80");
...
async function sendMessage(queuedPayload = null) {
  ...
  if (!queuedPayload && !grindActive && isFreshConv && isAbstractTask(text)) {
    const { dlgConfirm } = await import("../services/dialog.js?v=20260818-80");
...
function openCompressModal() {
  ...
  if (state.messages.length < 4) {
    import("../app.js?v=20260818-80").then(({ toast }) => toast("当前对话还不需要压缩"));
    return;
  }
...
function initChat() {
  ...
  expertSelect?.addEventListener("change", async () => {
    ...
    try {
      const { getExpert } = await import("../services/experts.js?v=20260818-80");
...
function startVoice(btn) {
  ...
  _voiceRecognition.onerror = (e) => {
    if (e.error !== "aborted" && e.error !== "no-speech") {
      try { import("../app.js?v=20260818-80").then(m => m.toast(t("语音识别错误: {err}", { err: e.error }))); } catch {}
    }
    stopVoice();
  };
