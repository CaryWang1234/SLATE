...
import { state, subscribe, addMessage, updateLastAssistantMessage, setMessages, setConversations, getModelKey, addUsage, estimateContextTokens, resetUsage, restoreUsageForConversation, setConversationUsage, setKnowledgeContext, savePersistent, getConversationTodos, setConversationTodos, setActiveExpertId, addBoardCard } from "../store.js?v=20260817-62";
import { get, post, del, patch, streamChat, upload, REASONING_PREFIX } from "../services/api.js?v=20260817-62";
import { buildMessages, getDefaultParams, getOutputMaxTokens } from "../services/adapter.js?v=20260817-62";
import { detectToolCalls, stripToolCalls, executeToolCalls, hasTruncatedTail } from "../services/tools.js?v=20260817-62";
import { renderMarkdown } from "../services/markdown.js?v=20260817-62";
import { openMemoryModal, openSnippetModal, autoRefineMemoryAndProfile, captureConversationSpark } from "./memory.js?v=20260817-62";
...
import { loadExperts, getExpert, readExpertFile } from "../services/experts.js?v=20260817-62";
import { fmtTokens, tokenEquivalence } from "../services/usage.js?v=20260817-62";
import { fileTypeIcon } from "../services/file_icons.js?v=20260817-62";
import { dlgConfirm, dlgPrompt, dlgToast } from "../services/dialog.js?v=20260817-62";
import * as grindSvc from "../services/grind.js?v=20260817-62";
...
async function continueTruncatedOutput(msgEl, content, modelId, apiKey, baseUrl, params, signal, finishReason = "") {
  ...
  for (let round = 1; round <= MAX_CONTINUE_ROUNDS; round++) {
    ...
    try {
      const { toast } = await import("../app.js?v=20260817-62");
...
function openCompressModal() {
  ...
  if (state.messages.length < 4) {
    import("../app.js?v=20260817-62").then(({ toast }) => toast("当前对话还不需要压�?));
    return;
  }
...
function initChat() {
  ...
  expertSelect?.addEventListener("change", async () => {
    ...
    try {
      const { getExpert } = await import("../services/experts.js?v=20260817-62");
