...
import { get, post, del, patch, streamChat } from "../services/api.js?v=20260919-002";
import { dlgConfirm, dlgPrompt } from "../services/dialog.js?v=20260919-002";
...
import { initVaultPanel, openVaultPanel } from "./vault.js?v=20260919-002";
  ...
  for (const mem of state.memories) {
    ...
    content.addEventListener("dblclick", async () => {
      ...
      if (newText !== null && newText.trim()) {
        ...
        catch (e) { import("../app.js?v=20260919-002").then(({ toast }) => toast(t("保存失败: {msg}", { msg: e.message }))); }
      }
    });
...
function initMemoryPanel() {
  ...
  if (btnSaveProfile) btnSaveProfile.addEventListener("click", () => {
    saveProfileFromForm();
    import("../app.js?v=20260919-002").then(({ toast }) => toast("资料已保存"));
  });
