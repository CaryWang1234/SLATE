...
import { state, getModelKey } from "../store.js?v=20260818-100";
...
async function startReview() {
  ...
  if (!model?.id || !apiKey) {
    import("../app.js?v=20260818-100").then(({ toast }) => toast("请先选择模型并配置 API Key")).catch(() => {});
    return;
  }
  ...
  try {
    ...
    renderReviewResult();
    showView("result");
    import("../app.js?v=20260818-100").then(({ toast }) => toast(t("代码审查完成"))).catch(() => {});
  } catch (e) {
    if (e.name === "AbortError") {
      ...
      import("../app.js?v=20260818-100").then(({ toast }) => toast(t("代码审查失败: {msg}", { msg: e.message }))).catch(() => {});
    }
  } finally {
    running = false;
    abortController = null;
...
async function copyToClipboard(text) {
  try {
    ...
    import("../app.js?v=20260818-100").then(({ toast }) => toast(t("已复制"))).catch(() => {});
  } catch {
    import("../app.js?v=20260818-100").then(({ toast }) => toast(t("复制失败"))).catch(() => {});
  }
}
