...
import { state, getModelKey } from "../store.js?v=20260830-001";
import { post, streamChat } from "../services/api.js?v=20260830-001";
import { renderMarkdown } from "../services/markdown.js?v=20260830-001";
import { t } from "../services/i18n.js?v=20260830-001";
import { setIconText } from "../services/icons.js?v=20260830-001";
...
async function startReview() {
  ...
  if (!model?.id || !apiKey) {
    import("../app.js?v=20260830-001").then(({ toast }) => toast("请先选择模型并配置 API Key")).catch(() => {});
    return;
  }
  ...
  try {
    ...
    renderReviewResult();
    showView("result");
    import("../app.js?v=20260830-001").then(({ toast }) => toast(t("代码审查完成"))).catch(() => {});
  } catch (e) {
    if (e.name === "AbortError") {
      ...
      import("../app.js?v=20260830-001").then(({ toast }) => toast(t("代码审查失败: {msg}", { msg: e.message }))).catch(() => {});
    }
  } finally {
    running = false;
    abortController = null;
...
async function copyToClipboard(text) {
  try {
    ...
    import("../app.js?v=20260830-001").then(({ toast }) => toast(t("已复制"))).catch(() => {});
  } catch {
    import("../app.js?v=20260830-001").then(({ toast }) => toast(t("复制失败"))).catch(() => {});
  }
}
