 ...
 * AI 全量扫描项目 首生成「导览·百科」与「规则手册首
 * 三档深度：简略/ 平衡 / 详细（扫描预算与输出长度随档位变化）
 */

import { state, setProject, getModelKey } from "../store.js?v=20260818-80";
import { post, streamChat } from "../services/api.js?v=20260818-80";
import { updateProjectConfig } from "../services/project.js?v=20260818-80";
...
function openUnderstandModal() {
  bindUnderstandModal();
  if (!state.project) {
    import("../app.js?v=20260818-80").then(({ toast }) => toast("请先打开一个项目")).catch(() => {});
    return;
  }
...
async function startUnderstanding() {
  ...
  if (!model?.id || !apiKey) {
    import("../app.js?v=20260818-80").then(({ toast }) => toast("请先选择模型并配色API Key")).catch(() => {});
    return;
  }
  ...
  try {
    ...
    renderResult();
    showView("result");
    import("../app.js?v=20260818-80").then(({ toast }) => toast("项目理解已生成")).catch(() => {});
  } catch (e) {
    if (e.name === "AbortError") {
      ...
      appendLog(t("首失败: {msg}", { msg: e.message }));
      import("../app.js?v=20260818-80").then(({ toast }) => toast(t("项目理解生成失败: {msg}", { msg: e.message }))).catch(() => {});
    }
  } finally {
    running = false;
...
function buildScanContext(scan) {
  const parts = [`项目名：${scan.project}`, "", "== 目录首==", scan.tree, "", "== 关键文件内容（截取开头） =="];
...
要求首
- 使用 Markdown，规则必须从扫描资料中真实存在的证据推导（如依赖文件、配置文件、代码风格、既有文档），禁止凭空捏首
- ${depth}
- 章节结构建
1. 技术边界：允许的依赖、构建方式、禁用的做法
2. 代码规范：从现有代码中观察到的命令结构/风格约定
3. 架构约束：模块划分、分层关系、扩展方首
...
async function copyCurrentResult() {
  ...
  try {
    ...
    import("../app.js?v=20260818-80").then(({ toast }) => toast("已复制到剪贴板")).catch(() => {});
  } catch {
    import("../app.js?v=20260818-80").then(({ toast }) => toast("复制失败")).catch(() => {});
  }
}
