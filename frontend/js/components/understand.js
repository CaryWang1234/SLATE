...
import { state, setProject, getModelKey } from "../store.js?v=20260808-16";
import { post, streamChat } from "../services/api.js?v=20260808-16";
import { updateProjectConfig } from "../services/project.js?v=20260808-16";
...
function openUnderstandModal() {
  bindUnderstandModal();
  if (!state.project) {
    import("../app.js?v=20260808-16").then(({ toast }) => toast("请先打开一个项目")).catch(() => {});
    return;
  }
...
async function startUnderstanding() {
  ...
  if (!model?.id || !apiKey) {
    import("../app.js?v=20260808-16").then(({ toast }) => toast("请先选择模型并配置 API Key")).catch(() => {});
    return;
  }
  ...
  try {
    ...
    appendLog(`开始扫描项目（档位：${LEVELS[currentLevel].label}）…`);
    ...
    appendLog(`扫描完成：${scan.total_files} 个文件，精读 ${scan.heads.length} 个${scan.truncated ? "（目录树已截断）" : ""}`);
    ...
    appendLog(`导览·百科完成（${tour.length} 字符）`);
    ...
    appendLog(`规则手册完成（${rules.length} 字符）`);
    ...
    if (saveRes.code === 0) {
      ...
      appendLog(`保存失败（仍可复制结果）: ${saveRes.message}`);
    }

    renderResult();
    showView("result");
    import("../app.js?v=20260808-16").then(({ toast }) => toast("项目理解已生成")).catch(() => {});
  } catch (e) {
    if (e.name === "AbortError") {
      ...
      import("../app.js?v=20260808-16").then(({ toast }) => toast(`项目理解生成失败: ${e.message}`)).catch(() => {});
    }
  } finally {
    running = false;
...
要求：
- 使用 Markdown，只依据扫描资料中的事实撰写，无法确定的内容明确标注"待确认"
- ${depth}
- 章节结构：
1. 项目概述：一句话定位 + 解决什么问题
2. 技术栈：语言/框架/依赖/构建方式
...
2. 代码规范：从现有代码中观察到的命名/结构/风格约定
3. 架构约束：模块划分、分层关系、扩展方式
4. 运行与构建命令：如何启动、测试、打包
- 每条规则一行，写成可直接执行的祈使句
- 直接输出文档正文，不要任何前言或解释`;
}
...
function renderResult() {
  ...
  if (resultMeta) resultMeta.textContent = `档位：${levelLabel} · 生成于 ${time}`;
  renderResultTabs();
  renderResultBody();
}
...
async function copyCurrentResult() {
  ...
  try {
    ...
    import("../app.js?v=20260808-16").then(({ toast }) => toast("已复制到剪贴板")).catch(() => {});
  } catch {
    import("../app.js?v=20260808-16").then(({ toast }) => toast("复制失败")).catch(() => {});
  }
}
