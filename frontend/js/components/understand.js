...
import { state, setProject, getModelKey } from "../store.js?v=20260925-007";
import { aiModelFor, aiFeatureBlocked } from "../services/ai_features.js?v=20260925-007";
import { post, streamChat, REASONING_PREFIX, REASONING_INLINE_PREFIX } from "../services/api.js?v=20260925-007";
import { updateProjectConfig } from "../services/project.js?v=20260925-007";
...
function openUnderstandModal() {
  bindUnderstandModal();
  if (!state.project) {
    import("../app.js?v=20260925-007").then(({ toast }) => toast("请先打开一个项目")).catch(() => {});
    return;
  }
...
async function startUnderstanding() {
  ...
  if (!target.usable) {
    import("../app.js?v=20260925-007").then(({ toast }) => toast(t("请先选择模型并配置 API Key"))).catch(() => {});
    return;
  }
  ...
  try {
    ...
    renderResult();
    showView("result");
    import("../app.js?v=20260925-007").then(({ toast }) => toast("项目理解已生成")).catch(() => {});
  } catch (e) {
    if (e.name === "AbortError") {
      ...
      import("../app.js?v=20260925-007").then(({ toast }) => toast(t("项目理解生成失败: {msg}", { msg: e.message }))).catch(() => {});
    }
  } finally {
    running = false;
...
async function copyCurrentResult() {
  ...
  try {
    ...
    import("../app.js?v=20260925-007").then(({ toast }) => toast("已复制到剪贴板")).catch(() => {});
  } catch {
    import("../app.js?v=20260925-007").then(({ toast }) => toast("复制失败")).catch(() => {});
  }
}
