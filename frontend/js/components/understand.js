...
import { state, setProject, getModelKey } from "../store.js?v=20260808-33";
import { post, streamChat } from "../services/api.js?v=20260808-33";
import { updateProjectConfig } from "../services/project.js?v=20260808-33";
...
function openUnderstandModal() {
  bindUnderstandModal();
  if (!state.project) {
    import("../app.js?v=20260808-33").then(({ toast }) => toast("请先打开一个项目")).catch(() => {});
    return;
  }
...
async function startUnderstanding() {
  ...
  if (!model?.id || !apiKey) {
    import("../app.js?v=20260808-33").then(({ toast }) => toast("请先选择模型并配置 API Key")).catch(() => {});
    return;
  }
  ...
  try {
    ...
    renderResult();
    showView("result");
    import("../app.js?v=20260808-33").then(({ toast }) => toast("项目理解已生成")).catch(() => {});
  } catch (e) {
    if (e.name === "AbortError") {
      ...
      import("../app.js?v=20260808-33").then(({ toast }) => toast(`项目理解生成失败: ${e.message}`)).catch(() => {});
    }
  } finally {
    running = false;
...
async function copyCurrentResult() {
  ...
  try {
    ...
    import("../app.js?v=20260808-33").then(({ toast }) => toast("已复制到剪贴板")).catch(() => {});
  } catch {
    import("../app.js?v=20260808-33").then(({ toast }) => toast("复制失败")).catch(() => {});
  }
}
