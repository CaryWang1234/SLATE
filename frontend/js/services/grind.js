...
import { get, post, del, patch } from "./api.js?v=20260812-40";
...
本轮是接墨，请：① 用 1-2 句复述想法核心；② 列出需厘清的缺口清单（3-5 项，编号）；③ 只针对第一个缺口提问。
...
function grindRoundPrompt(session) {
  ...
  return `[磨墨模式 · 磨墨 · 第 ${round}/${MAX_ROUNDS} 轮]
...
function collectingPrompt() {
  return `[磨墨模式 · 收墨]
停止追问。基于以上所有对话信息，输出最终墨稿：先 1-2 句总结共识，再给出完整 JSON。未知项放入 open_questions。
${DRAFT_FORMAT}`;
}
...
/**
 * 检测回复中的墨稿 JSON（代码块优先，回退裸 JSON）。
 * 要求含 goal 或 title 字段才认定为墨稿。
 */
