...
import { get, post, del, patch } from "./api.js?v=20260814-48";
...
const RULES = [
  "追问纪律：每条回复只问一个缺口；能给 A/B 选项就给选项（选项简短、互斥），开放式问题最少用�?,
  "不要寒暄、不要重复已确认内容、不要一次问多个问题�?,
  "用户回复「收�?/ 够了 / 就这样」或信息已足够时，立即进入收墨输出墨稿�?,
].join("\n");
...
function firstRoundPrompt(idea) {
  return `[磨墨模式 · 接墨]
你现在是 SLATE 的磨墨助手：通过有节奏的追问，把粗糙想法研磨成可执行的任务书（墨稿）。最多追�?${MAX_ROUNDS} 轮�?
${RULES}
${STATUS_FORMAT}
${DRAFT_FORMAT}

本轮是接墨，请：�?�?1-2 句复述想法核心；�?列出需厘清的缺口清单（3-5 项，编号）；�?只针对第一个缺口提问�?
...
function collectingPrompt() {
  return `[磨墨模式 · 收墨]
停止追问。基于以上所有对话信息，输出最终墨稿：�?1-2 句总结共识，再给出完整 JSON。未知项放入 open_questions�?
${DRAFT_FORMAT}`;
}
...
/** 墨稿 �?白板卡片（结构与 whiteboard.js saveCard 一致） */
function draftToBoardCard(draft) {
  return {
    id: `c${Date.now().toString(36)}`,
    title: `墨稿 · ${draft.title || "未命�?}`,
