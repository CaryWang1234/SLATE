...
import { state, getModelKey } from "../store.js?v=20260808-33";
import { post } from "./api.js?v=20260808-33";
...
async function explainCommand(command) {
  ...
  if (!modelId || !apiKey) return "（当前未配置模型或 API Key，无法生成目的说明）";
  try {
 ...
 * 批准 → 向 params 注入 approved 并返回 true；拒绝 → 返回 false
 */
