...
import { state, getModelKey } from "../store.js?v=20260817-58";
import { post } from "./api.js?v=20260817-58";
...
const HIGH_RISK_PATTERNS = [
  ...
  { re: /reg\s+(delete|add)\b/i, reason: "修改注册�? },
  ...
  { re: /git\s+push\s+[^;]*(--force\b|-f\b|--force-with-lease)/i, reason: "Git 强制推�? },
  { re: /git\s+reset\s+--hard/i, reason: "Git 硬重置（丢弃改动�? },
  { re: /git\s+clean\s+-[a-z]*f/i, reason: "Git 清理未跟踪文�? },
  { re: /git\s+branch\s+-D\b/i, reason: "Git 强制删除分支" },
  { re: /(drop\s+(database|table|schema)|truncate\s+table)/i, reason: "数据库删�?删库" },
...
async function explainCommand(command) {
  ...
  try {
    ...
    return t("（说明生成失�? {msg}�?, { msg: e.message });
  }
}
...
/**
 * 统一守卫入口：若�?terminal 且命令高危则弹框审批�?
 * 批准 �?�?params 注入 approved 并返�?true；拒�?�?返回 false
 */
