...
// 双语文案齐备（与分项目宪法同一套说法）
for (const key of [
  "正在编辑项目「{name}」的 Action：存进该项目的 .slate/actions/，同名时顶掉全局那份",
  "正在编辑全局 Action：打开带同名 Action 的项目时，以该项目 .slate/actions/ 里那份为准",
  "这条知识存哪一份？存进项目「{name}」只在这个项目里可见（同名时顶掉全局那条）；点「存为全局」则所有项目共用。",
  "只在这个项目里不用它，别的项目照旧",
  "摘掉后这台服务器回到跟全局那份",
  "以下是项目那一份的留底。",
]) {
